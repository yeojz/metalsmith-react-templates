...
const assertFileContents = function(expected, built){
    const expectedContents = fs.readFileSync(expected, 'utf-8');
    const buildContents = fs.readFileSync(built, 'utf-8');

    assert.strictEqual(expectedContents, buildContents);
}

const fixturesRel = function(location){
    return path.join('..', location);
}

const fixtures = function(location){
...
describe('metalsmith-react-templates', function(){
    ...
    it('should render functional templates (react 0.14+)', function(){
        new Metalsmith('test/fixtures/render-functional-templates')
            .source(fixturesRel('basic/src'))
            .destination(fixturesRel('results/render-functional-templates'))
            .use(templates({
                html: true
            }))
            .build(function(err){
                ...
                assertFileContents(
                    fixtures('render-functional-templates/expected/source.html'),
                    fixtures('results/render-functional-templates/source.html')
                );
